...
contract AttestedSnapshotVerifier is ITrack2Verifier {
    ...
    event AttestedSnapshotUsed(address indexed user, bytes32 indexed datasetId, uint256 score);
    ...
    constructor(address attester, address systemAddress) {
    ...
    function verifyAndScore(
        ...
        if (recovered != ATTESTER) revert InvalidSignature();

        score = _scoreForTier(tier);
        valid = true;

        emit AttestedSnapshotUsed(user, datasetId, score);
    }
        ...
        returns (bytes32)
    {
        ...
        bytes memory nativeAccountId = SYSTEM.toAccountId(user);

        return keccak256(
            abi.encodePacked(
                "PolkaZK Credit Snapshot Certificate",
                block.chainid,
                address(this),
                user,
                nativeAccountId,
